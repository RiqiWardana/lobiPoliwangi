...
<body>
    ...
    <div class="pl-64 pr-6 w-full h-full">
        ...
        <form class="font-poppins mt-4" action="/dashboard-admin/index-data-lomba/edit-data/{{ $data->id }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="flex flex-row justify-between w-8/12">
                <div>
                    <label class="text-sm font-medium text-black">Nama Lomba</label><br>
                    <input class="w-80 outline-0 border-[1px] rounded-md border-black pl-2 pr-2 text-xs leading-8 placeholder:text-xs placeholder:font-extralight placeholder:italic" type="text" name="namaLomba" placeholder="Masukan Nama Lomba" value="{{ $data->nama_lomba }}">
                </div>
                <div class="mr-12">
                    <label class="text-sm font-medium text-black">Nama Instansi</label><br>
                    <input class="w-64 outline-0 border-[1px] rounded-md border-black pl-2 pr-2 text-xs leading-8 placeholder:text-xs placeholder:font-extralight placeholder:italic" type="text" name="namaInstansi" placeholder="Masukan Nama Instansi Etc. PT. Sumber Makmr" value="{{ $data->instansiLomba->nama_instansi_lomba }}">
                </div>
            </div>
            ...
            <div class="flex flex-row">
                <div class="mr-12 mt-4">
                    ...
                    <input class="w-64 mt-2 bg-sky-500 outline-0 border-0 rounded-md border-black pl-2 pr-2 text-white text-xs leading-8 placeholder:text-xs placeholder:font-extralight placeholder:italic" type="date" name="tanggalPendaftaranTerakhir" placeholder="Masukan Nama Instansi Etc. PT. Sumber Makmr" value="{{ $data->tanggal_penutupan }}">
                </div>
                <div class="mr-12 mt-4">
            ...
            <input type="hidden" name="idInstansiOldLomba" value="{{ $data->instansiLomba->id }}">
