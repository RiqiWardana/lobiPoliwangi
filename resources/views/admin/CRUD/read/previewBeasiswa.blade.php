...
<body>
    ...
    <div class="pl-64 pr-4 pt-3 w-full h-full font-poppins">
        ...
        <!-- Bagian Informasi -->
        <div>
            <p class="text-3xl font-bold mt-10">{{ $data->nama_beasiswa }}</p>
            <p class="w-fit h-fit p-1.5 pl-4 pr-4 bg-blue-500 rounded-lg text-sm text-slate-100 font-medium">{{ $data->instansiBeasiswa->nama_instansi_beasiswa }}</p>
        </div>
        <div class="mt-6">
        ...
        <div class="w-5/12 flex flex-row mt-10 mb-10 justify-between">
            ...
            <a href="/dashboard-admin/index-data-beasiswa/edit-data/{{ $data->id }}" class="w-fit h-fit p-3 bg-slate-300 rounded cursor-pointer" type="submit">Edit Data Ini</a>
            <form action="/dashboard-admin/index-data-beasiswa/deleting-data/{{ $data->id }}" method="post">
                @csrf
                @method('delete')
                <input type="hidden" name="idInstansiBeaisiswa" value="{{ $data->instansiBeasiswa->id }}">
