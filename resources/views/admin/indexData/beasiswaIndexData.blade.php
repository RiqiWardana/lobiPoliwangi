...
<body>
    ...
    <div class="pl-64 pr-6 w-full h-full">
        <table class="table-auto border-2 font-poppins w-full">
            <thead>
                <tr>
                    ...
                    <td class="bg-slate-300 font-semibold text-center">Nama Beasiswa</td>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="border p-4">{{ $data->nama_beasiswa }}</td>
                    <td class="border p-4">{{ $data->instansiBeasiswa->nama_instansi_beasiswa }}</td>
                    ...
                    <td class="border p-2">
                        <div class="w-full h-full gap-4 flex flex-col justify-between items-center">
                            <a href="/dashboard-admin/index-data-beasiswa/preview/{{ $data->id }}" class="w-fit h-fit p-2 flex justify-center items-center bg-slate-200 rounded-md bg-gradient-to-r from-cyan-500 to-blue-500 text-white text-sm">Preview</a>
                            <form class="w-fit h-fit inline" action="/dashboard-admin/index-data-beasiswa/deleting-data/{{ $data->id }}" method="post">
                                @csrf
                                @method('delete')
                                <input type="hidden" name="idInstansiBeasiswa" value="{{ $data->instansiBeasiswa->id }}">
        ...
        <!-- Tombol Tambah Data -->
        <a href="/dashboard-admin/tambah-data-beasiswa" class="w-16 h-16 bg-sky-300 absolute bottom-6 right-6 rounded-full cursor-pointer hover:w-48">
        </a>
    </div>
    </div>
