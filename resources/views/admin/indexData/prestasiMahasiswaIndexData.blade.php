...
<head>
    ...
    <title>Index Data Prestasi Mahasiswa | Lobi Poliwangi</title>
</head>
<body>
    ...
    <div class="pl-64 pr-6 w-full h-full">
        <table class="table-auto border-2 font-poppins w-full">
            <thead>
                <tr>
                    ...
                    <td class="bg-slate-300 font-semibold text-center">Nama Prestasi</td>
                    <td class="bg-slate-300 font-semibold text-center">Prestasi</td>
            ...
            <tbody>
                ...
                @else
                @foreach ($dataPrestasiAll as $data)
                <tr>
                    <!-- Tempat Foto -->
                    ...
                    <td class="border p-4">{{ $data->prestasi->nama_perlombaan_prestasi }}</td>
                    <td class="border p-4">{{ $data->prestasi->nama_prestasi }}</td>
                    <td class="border p-2">
                        <div class="w-full h-full gap-4 flex flex-col justify-between items-center">
                            <a href="/dashboard-admin/index-data-prestasi-mahasiswa/preview/{{ $data['nim'] }}" class="w-fit h-fit p-2 flex justify-center items-center bg-slate-200 rounded-md bg-gradient-to-r from-cyan-500 to-blue-500 text-white text-sm">Preview</a>
                        </div>
                    </td>
                </tr>
                @endforeach
        ...
        <!-- Tombol Tambah Data -->
        <a href="/dashboard-admin/tambah-data-prestasi-mahasiswa" class="w-16 h-16 bg-sky-300 absolute bottom-6 right-6 rounded-full cursor-pointer hover:w-48">
        </a>
    </div>
    </div>
    ...
    <script>alert('{{ session("wrongCredentials") }}')</script>
    @endif
</body>
</html>